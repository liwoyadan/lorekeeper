    ...
    <div class="media-body row mw-100 mx-0" style="flex:1;flex-wrap:wrap;">
        ...
        <div class="d-block" style="flex:1">
            ...
            <div class="comment border pb-2 px-3 rounded {{ $limit == 0 ? 'shadow-sm border-info' : '' }} {{ $comment->is_featured && ($limit != 0) ? 'border-success' : '' }} {{ $comment->likes()->where('is_like', 1)->count() -$comment->likes()->where('is_like', 0)->count() < 0 ? 'bg-light bg-gradient' : '' }}">
                ...
                <div class="border-top pt-1 text-right text-muted small d-flex justify-content-end">
                    {!! $comment->created_at !!}
                    @if ($comment->created_at != $comment->updated_at)
                        <span class="text-muted border-left px-1">
                    ...
                    <a href="{{ url('reports/new?url=') . $comment->url }}">
                        <i class="fas fa-exclamation-triangle" data-toggle="tooltip" title="Click here to report this comment." style="opacity: 50%;"></i>
                    </a>
                </div>
            </div>
