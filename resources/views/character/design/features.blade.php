...
@section('design-content')
    ...
    @if ($request->status == 'Draft' && $request->user_id == Auth::user()->id)
        ...
        {{ html()->form('POST', 'designs/' . $request->id . '/traits')->open() }}
        <div class="form-group">
            {{ html()->label('Species', 'species_id') }}
            ...
            @else
                {{ html()->select('species_id', $specieses, $request->species_id)->class('form-control')->id('species')->placeholder('Select Species') }}
            @endif

        </div>

        <div class="form-group">
            {{ html()->label('Species Subtype(s)', 'subtype_ids') }}
            ...
            @else
                <div id="subtypes">
                    {{ html()->select('subtype_ids[]', $subtypes, $request->subtypes())->class('form-control')->id('subtype')->attribute('multiple', true) }}
                </div>
            @endif
        </div>

        <div class="form-group">
            {{ html()->label('Character Rarity', 'rarity_id') }}
            ...
            @else
                {{ html()->select('rarity_id', $rarities, $request->rarity_id)->class('form-control')->id('rarity')->placeholder('Select Rarity') }}
            @endif
        </div>
                ...
                @if ($request->character->is_myo_slot && $request->character->image->features)
                    @foreach ($request->character->image->features as $feature)
                        <div class="mb-2 d-flex align-items-center">
                            {{ html()->text('', $feature->name)->class('form-control mr-2')->attribute('disabled', true) }}
                            {{ html()->text('', $feature->data)->class('form-control mr-2')->attribute('disabled', true) }}
                ...
                @if ($request->features)
                    @foreach ($request->features as $feature)
                        <div class="mb-2 d-flex">
                            {{ html()->select('feature_id[]', $features, $feature->feature_id)->class('form-control mr-2 initial feature-select')->placeholder('Select Trait') }}
