...
@section('design-content')
    ...
    @if (($request->status == 'Draft' && $request->user_id == Auth::user()->id) || ($request->status == 'Pending' && Auth::user()->hasPower('manage_characters')))
        ...
        @endif
        {{ html()->form('POST', 'designs/' . $request->id . '/image')->enctype('multipart/form-data')->open() }}
        @if ($request->status == 'Draft' && $request->user_id == Auth::user()->id)
            <div class="form-group">
                {{ html()->label('Image') }} {!! add_help('This is the image that will be used on the masterlist. Note that the image is not protected in any way, so take precautions to avoid art/design theft.') !!}
        ...
        @else
            <div class="form-group">
                {{ html()->checkbox('modify_thumbnail', 0, 1)->class('form-check-input')->data('toggle', 'toggle') }}
                {{ html()->label('Modify Thumbnail', 'modify_thumbnail')->class('form-check-label ml-3') }} {!! add_help('Toggle this option to modify the thumbnail, otherwise only the credits will be saved.') !!}
            </div>
        @endif
