<ul>
    ...
    @if (Auth::check() && (Auth::user()->id == $character->user_id || Auth::user()->hasPower('manage_characters')))
        <li class="sidebar-section">
            ...
            <div class="sidebar-item"><a href="{{ $character->url . '/' . __('links.links') . '/edit' }}" class="{{ set_active('character/' . $character->slug . '/' . __('links.links') . '/edit') }}">Request {{ ucfirst(__('links.links')) }}</a>
            </div>
