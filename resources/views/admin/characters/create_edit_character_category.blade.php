...
@section('admin-content')
    ...
    {{ html()->form('POST', $category->id ? 'admin/data/character-categories/edit/' . $category->id : 'admin/data/character-categories/create')->acceptsFiles()->open() }}
    ...
    <div class="form-group">
        {{ html()->label('Code') }} {!! add_help('This is used in generating the codename for the character. Choose a short unique identifier, e.g. MYO, GUEST, etc.') !!}
        {{ html()->text('code', $category->code)->class('form-control') }}
    </div>

    <div class="form-group">
        {{ html()->label('Sub Masterlist (Optional)') }} {!! add_help('This puts it onto a sub masterlist.') !!}
        {{ html()->select('masterlist_sub_id', $sublists, $category->masterlist_sub_id)->class('form-control') }}
    </div>
    ...
    <div class="form-group">
        ...
        {{ html()->label('Is Visible', 'is_visible')->class('form-check-label ml-3') }} {!! add_help('If turned off, the category will not be visible in the category list or available for selection in search. Permissioned staff will still be able to add characters to them, however.') !!}
    </div>
