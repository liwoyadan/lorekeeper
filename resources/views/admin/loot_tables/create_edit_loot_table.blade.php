...
@section('admin-content')
    ...
    {{ html()->form('POST', $table->id ? 'admin/data/loot-tables/edit/' . $table->id : 'admin/data/loot-tables/create')->open() }}

    <h3>Basic Information</h3>

    <div class="form-group">
        {{ html()->label('Name') }} {!! add_help('This is the name you will use to identify this table internally. This name will not be shown to users and does not have to be unique, but a name that can be easily identified is recommended.') !!}
        {{ html()->text('name', $table->name)->class('form-control') }}
    </div>

    <div class="form-group">
        {{ html()->label('Display Name') }} {!! add_help('This is the name that will be shown to users, for example when displaying the rewards for doing a prompt. This is for display purposes and can be something more vague than the above, e.g. "A Random Rare Item"') !!}
        {{ html()->text('display_name', $table->getRawOriginal('display_name'))->class('form-control') }}
    </div>
                    ...
                    <tr class="loot-row">
                        ...
                        <td class="loot-row-select">
                            @if ($loot->rewardable_type == 'Item')
                                {{ html()->select('rewardable_id[]', $items, $loot->rewardable_id)->class('form-control item-select selectize')->placeholder('Select Item') }}
                            @elseif($loot->rewardable_type == 'ItemRarity')
                                <div class="item-rarity-select d-flex">
                                    {{ html()->select('criteria[]', ['==' => '=', '<' => '<', '>' => '>', '<=' => '<=', '>=' => '>='], isset($loot->data['criteria']) ? $loot->data['criteria'] : null)->class('form-control')->placeholder('Criteria') }}
                                    {{ html()->select('rarity[]', $rarities, isset($loot->data['rarity']) ? $loot->data['rarity'] : null)->class('form-control')->placeholder('Rarity') }}
                                </div>
                            @elseif($loot->rewardable_type == 'Currency')
                                {{ html()->select('rewardable_id[]', $currencies, $loot->rewardable_id)->class('form-control currency-select selectize')->placeholder('Select Currency') }}
                            @elseif($loot->rewardable_type == 'LootTable')
                                {{ html()->select('rewardable_id[]', $tables, $loot->rewardable_id)->class('form-control table-select selectize')->placeholder('Select Loot Table') }}
                            @elseif($loot->rewardable_type == 'ItemCategoryRarity')
                                <div class="category-rarity-select d-flex">
                                    {{ html()->select('rewardable_id[]', $categories, $loot->rewardable_id)->class('form-control selectize')->placeholder('Category') }}
                                    {{ html()->select('criteria[' . $loop->index . ']', ['==' => '=', '<' => '<', '>' => '>', '<=' => '<=', '>=' => '>='], isset($loot->data['criteria']) ? $loot->data['criteria'] : null)->class('form-control')->placeholder('Criteria') }}
                                    {{ html()->select('rarity[' . $loop->index . ']', $rarities, isset($loot->data['rarity']) ? $loot->data['rarity'] : null)->class('form-control')->placeholder('Rarity') }}
                                </div>
                            @elseif($loot->rewardable_type == 'ItemCategory')
                                {{ html()->select('rewardable_id[]', $categories, $loot->rewardable_id)->class('form-control item-select selectize')->placeholder('Select Item') }}
                            @elseif($loot->rewardable_type == 'None')
                                {{ html()->select('rewardable_id[]', [1 => 'No reward given.'], $loot->rewardable_id)->class('form-control') }}
                            @endif
                        </td>
                        <td>{{ html()->text('quantity[]', $loot->quantity)->class('form-control') }}</td>
                        <td class="loot-row-weight">{{ html()->text('weight[]', $loot->weight)->class('form-control loot-weight') }}</td>
    ...
    <div class="text-right">
        {{ html()->submit($table->id ? 'Edit' : 'Create')->class('btn btn-primary') }}
    </div>
        ...
        <table class="table table-sm">
            <tbody id="lootRow">
                <tr class="loot-row">
                    ...
                    <td>{{ html()->text('quantity[]', 1)->class('form-control') }}</td>
                    <td class="loot-row-weight">{{ html()->text('weight[]', 1)->class('form-control loot-weight') }}</td>
                    <td class="loot-row-chance"></td>
                    <td class="text-right"><a href="#" class="btn btn-danger remove-loot-button">Remove</a></td>
                </tr>
            </tbody>
        </table>
        {{ html()->select('rewardable_id[]', $items, null)->class('form-control item-select')->placeholder('Select Item') }}
        <div class="item-rarity-select d-flex">
            {{ html()->select('criteria[]', ['==' => '=', '<' => '<', '>' => '>', '<=' => '<=', '>=' => '>='], null)->class('form-control criteria-select')->placeholder('Criteria') }}
            {{ html()->select('rarity[]', $rarities, null)->class('form-control criteria-select')->placeholder('Rarity') }}
        </div>
        {{ html()->select('rewardable_id[]', $currencies, null)->class('form-control currency-select')->placeholder('Select Currency') }}
        {{ html()->select('rewardable_id[]', $tables, null)->class('form-control table-select')->placeholder('Select Loot Table') }}
        {{ html()->select('rewardable_id[]', $categories, null)->class('form-control category-select')->placeholder('Select Item Category') }}
        <div class="category-rarity-select d-flex">
            {{ html()->select('rewardable_id[]', $categories, null)->class('form-control')->placeholder('Category') }}
            {{ html()->select('criteria[]', ['==' => '=', '<' => '<', '>' => '>', '<=' => '<=', '>=' => '>='], null)->class('form-control criteria-select')->placeholder('Criteria') }}
            {{ html()->select('rarity[]', $rarities, null)->class('form-control criteria-select')->placeholder('Rarity') }}
        </div>
        {{ html()->select('rewardable_id[]', [1 => 'No reward given.'], null)->class('form-control none-select') }}
    </div>
    ...
    @if ($table->id)
        ...
        <div class="form-group">
            {{ html()->label('Number of Rolls', 'quantity') }}
            {{ html()->text('quantity', 1)->class('form-control')->id('rollQuantity') }}
        </div>
        <div class="text-right">
