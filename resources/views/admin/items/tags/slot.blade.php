...
<div class="form-group">
    ...
    {{ html()->text('name', $tag->getData()['name'])->class('form-control') }}
</div>
...
<div class="form-group">
    ...
    @endif
    {{ html()->textarea('description', $tag->getData()['description'])->class('form-control wysiwyg') }}
</div>

<div class="form-group">
    {{ html()->checkbox('is_visible', $tag->getData()['is_visible'], 1)->class('form-check-input')->attribute('data-toggle', 'toggle') }}
...
<div class="form-group">
    {{ html()->checkbox('is_giftable', $tag->getData()['is_giftable'], 1)->class('form-check-input')->attribute('data-toggle', 'toggle') }}
    {{ html()->label('Is Giftable', 'is_giftable')->class('form-check-label ml-3') }}
</div>
<div class="form-group">
    {{ html()->checkbox('is_tradeable', $tag->getData()['is_tradeable'], 1)->class('form-check-input')->attribute('data-toggle', 'toggle') }}
    {{ html()->label('Is Tradeable', 'is_tradeable')->class('form-check-label ml-3') }}
</div>
<div class="form-group">
    {{ html()->checkbox('is_sellable', $tag->getData()['is_sellable'], 1)->class('form-check-input')->attribute('data-toggle', 'toggle')->id('resellable') }}
    {{ html()->label('Is Resellable', 'is_sellable')->class('form-check-label ml-3') }}
</div>
<div class="card mb-3" id="resellOptions">
    <div class="card-body">
        {{ html()->label('Resale Value') }} {!! add_help('This value is publicly displayed on the MYO slot\'s page.') !!}
        {{ html()->text('sale_value', $tag->getData()['sale_value'])->class('form-control') }}
    </div>
</div>
...
<div class="form-group">
    {{ html()->label('Species') }} {!! add_help('This will lock the slot into a particular species. Leave it blank if you would like to give the user a choice.') !!}
    {{ html()->select('species_id', $specieses, $tag->getData()['species_id'])->class('form-control')->id('species') }}
</div>

<div class="form-group" id="subtypes">
    {{ html()->label('Subtypes (Optional)') }} {!! add_help(
    ...
    {{ html()->select('subtype_ids[]', $subtypes, $tag->getData()['species_id'] ? $tag->getData()['subtype_ids'] : null)->class('form-control')->placeholder('Select Species First')->id('subtype')->attribute('multiple', 'multiple') }}
</div>

<div class="form-group">
    {{ html()->label('Character Rarity') }} {!! add_help('This will lock the slot into a particular rarity. Leave it blank if you would like to give the user more choices.') !!}
    {{ html()->select('rarity_id', $rarities, $tag->getData()['rarity_id'])->class('form-control') }}
</div>
